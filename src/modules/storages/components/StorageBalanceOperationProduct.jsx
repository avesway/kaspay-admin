...
const typesBalances = [
  ...
  { type: 'theft', name: 'Воровство' },
  { type: 'expiration', name: 'Просрочен' },
];

const StorageBalanceOperationProduct = ({ product }) => {
  ...
  const productSchema = z.object({
    ...
    operationType: z.string().min(1, 'Укажите тип баланса'),
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Операции с балансом</DialogTitle>
          <DialogDescription>Укажите количество и операцию</DialogDescription>
        </DialogHeader>
                ...
                render={({ field: { onChange, value } }) => (
                  <FormItem>
                    <Select value={value} onValueChange={onChange} disabled>
                      <FormLabel className="gap-1">Текущее расположение</FormLabel>
                ...
                render={({ field: { onChange, value } }) => (
                  <FormItem>
                    <Select value={value} onValueChange={onChange}>
                      <FormLabel className="gap-1">
                        Тип баланса <span className="text-destructive">*</span>
                      </FormLabel>
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder="Укажите тип">
                          {value && typesBalances.find((war) => war.type == value)?.name}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                        <SelectGroup>
                          {typesBalances.map((storage) => (
...
export default StorageBalanceOperationProduct;
